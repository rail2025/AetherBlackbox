...
public class AetherBlackboxWindow : Window {
    ...
    public AetherBlackboxWindow(Plugin plugin) : base("Aether Blackbox") {
        this.plugin = plugin;
    ...
    public override void Draw() {
        try {
            ...
            ImGui.SameLine(0, 15);
            if (ImGuiComponents.IconButton("VisibilityButton", FontAwesomeIcon.Eye))
                ImGui.OpenPopup("replay_visibility_settings");
            if (ImGui.IsItemHovered())
                ImGui.SetTooltip("Replay Visibility Settings");

            if (ImGui.BeginPopup("replay_visibility_settings"))
            {
                ImGui.TextUnformatted("Replay Visualization");

                var showNpcs = plugin.Configuration.ShowReplayNpcs;
                if (ImGui.Checkbox("Show NPCs/Objects", ref showNpcs))
                {
                    plugin.Configuration.ShowReplayNpcs = showNpcs;
                    plugin.Configuration.Save();
                }

                var showHp = plugin.Configuration.ShowReplayHp;
                if (ImGui.Checkbox("Show HP Bars", ref showHp))
                {
                    plugin.Configuration.ShowReplayHp = showHp;
                    plugin.Configuration.Save();
                }
            ...
            if (death != null && death.ReplayData.Frames.Count > 0)
            {
                ImGui.SameLine(0, 15);
            ...
            if (ImGui.Button(config))
                plugin.RecapConfigWindow.IsOpen = true;
    ...
    private void DrawCombatEventTable(Death? death) {
        if (ImGui.BeginTable("AetherBlackbox", 6,
