    ...
    public static class DrawableSerializer
    {
        ...
        public static List<BaseDrawable> DeserializePageFromBytes(byte[] data)
        {
            ...
            using (var reader = new BinaryReader(memoryStream))
            {
                ...
                {
                    Service.PluginLog?.Error(e, "Deserialization failed due to end of stream. The data may be corrupt or incomplete.");
                }
                catch (Exception e)
                {
                    Service.PluginLog?.Error(e, "An unexpected error occurred during deserialization.");
                }
            }
            return deserializedDrawables;
        }
        ...
        private static void SerializeSingleDrawable(BinaryWriter writer, BaseDrawable drawable)
        {
            ...
            switch (drawable.ObjectDrawMode)
            {
                ...
                case DrawMode.Laser:
                    ...
                    break;
                default:
                    Service.PluginLog?.Error($"[DrawableSerializer] Unhandled DrawMode during serialization: {drawable.ObjectDrawMode}");
                    break;
            }
        }
        ...
        private static BaseDrawable? DeserializeSingleDrawable(BinaryReader reader, int version)
        {
            ...
            switch (mode)
            {
                ...
                case DrawMode.Laser:
                    ...
                    {
                        Service.PluginLog?.Error($"[DrawableSerializer] Invalid laser point count: {laserPointCount}.");
                        return null;
                    }
                    ...
                    break;
                default:
                    Service.PluginLog?.Error($"[DrawableSerializer] Unhandled DrawMode during deserialization: {mode}");
                    break;
            }
