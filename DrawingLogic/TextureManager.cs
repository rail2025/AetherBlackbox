...
namespace AetherBlackbox.DrawingLogic
{
    public static class TextureManager
    {
        ...
        {
            if (LoadedImageData.TryGetValue(resourcePath, out var data)) return data;

            // Synchronous fallback for local embedded resources required by background exporters
            if (!resourcePath.StartsWith("http", StringComparison.OrdinalIgnoreCase) && !resourcePath.StartsWith("emoji:") && !resourcePath.StartsWith("luminaicon:"))
            {
                try
                {
                    var assembly = Assembly.GetExecutingAssembly();
                    var targetName = $"{assembly.GetName().Name}.{resourcePath.Replace("\\", ".").Replace("/", ".")}";
                    var foundResource = assembly.GetManifestResourceNames().FirstOrDefault(n => n.Equals(targetName, StringComparison.OrdinalIgnoreCase));
                    ...
                    if (foundResource != null)
                    {
                        ...
                        if (stream != null)
                        {
                            var rawBytes = ReadStream(stream);
                            byte[]? finalBytes = rawBytes;

                            if (resourcePath.EndsWith(".svg", StringComparison.OrdinalIgnoreCase))
                            {
                                using var svgStream = new MemoryStream(rawBytes);
                                finalBytes = RasterizeSvg(svgStream);
                            }
                            ...
                            if (finalBytes != null)
                            {
                                LoadedImageData.TryAdd(resourcePath, finalBytes);
                                return finalBytes;
                            }
                        }
                    }
                ...
                {
                    Service.PluginLog?.Error(ex, $"[TextureManager] Synchronous fallback load failed for: {resourcePath}");
                }
            }
