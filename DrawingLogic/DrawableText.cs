...
namespace AetherBlackbox.DrawingLogic
{
    public class DrawableText : BaseDrawable
    {
        ...
        public override void DrawToImage(IImageProcessingContext context, Vector2 canvasOriginInOutputImage, float currentGlobalScale)
        {
            ...
            catch (Exception ex) { AetherBlackbox.Service.PluginLog?.Warning(ex, "[DrawableText.DrawToImage] Error loading system font."); }
            if (font == null)
            {
                AetherBlackbox.Service.PluginLog?.Error("[DrawableText.DrawToImage] Font not loaded, cannot draw text to image.");
