...
namespace AetherBlackbox.DrawingLogic
{
    public class DrawableImage : BaseDrawable
    {
        ...
        public override void DrawToImage(IImageProcessingContext context, Vector2 canvasOriginInOutputImage, float currentGlobalScale)
        {
            ...
            if (imageBytes == null)
            {
                Service.PluginLog?.Warning($"[DrawableImage.DrawToImage] Image data for '{this.ImageResourcePath}' was not available in the cache for export.");
                return;
            }
            ...
            {
                Service.PluginLog?.Error(ex, $"Failed to process and draw image for export: {this.ImageResourcePath}");
            }
        }
