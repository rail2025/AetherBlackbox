...
namespace AetherBlackbox.Windows
{
    public class PropertiesWindow : Window, IDisposable
    {
        ...
        public PropertiesWindow(Plugin plugin) : base("Properties###AetherBlackboxProperties")
        {
            this.plugin = plugin;
        ...
        // Find which list a specific DrawMode belongs to
        ...
        public override void Draw()
        {
            ...
            if (ImGui.Checkbox("Locked", ref isLocked))
            {
                ...
                // Commit immediately
                mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), isLocked ? "Lock Objects" : "Unlock Objects");
                    ...
                    for (int i = 0; i < jobList.Count; i++)
                    {
                        ...
                        if (tex != null)
                        {
                            if (ImGui.ImageButton(tex.Handle, btnVec))
                            {
                                ...
                                mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), "Swap Job Icon");
                                mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable>(selected));
                            }
            ...
            for (int i = 0; i < ColorPalette.Length; i++)
            {
                ...
                if (ImGui.ColorButton($"##PropColor{i}", ColorPalette[i], ImGuiColorEditFlags.NoTooltip | ImGuiColorEditFlags.NoAlpha, colorButtonDimensions))
                {
                    ...
                    mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), "Change Color");
                    mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable>(selected));
                }
                ImGui.PopID();
            ...
            {
                mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), "Change Opacity");
                mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable>(selected));
            }
            ...
            foreach (var t in ThicknessPresets)
            {
                ...
                if (ImGui.Button($"{t:0}##PropThick{t}", new Vector2(thicknessButtonWidth, 0)))
                {
                    ...
                    mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), "Change Thickness");
                    mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable>(selected));
                }
            ...
            if (ImGui.Checkbox("Filled", ref isFilled))
            {
                ...
                mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), "Change Fill");
                mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable>(selected));
            }
            ...
            if (selected.Count == 1 && selected[0] is DrawableDonut donut)
            {
                ...
                if (ImGui.IsItemDeactivatedAfterEdit())
                {
                    mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), "Resize Donut Hole");
                    mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable>(selected));
                }
            }
            ...
            if (selected.Count == 1 && selected[0] is DrawableText textObj)
            {
                ...
                if (ImGui.IsItemDeactivatedAfterEdit())
                {
                    mainWindow.CanvasController.UndoManager.RecordAction(mainWindow.PageManager.GetCurrentPageDrawables(), "Change Font Size");
                    mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable>(selected));
                }
            ...
            if (selected.Count == 1)
            {
                ...
                if (ImGui.Button("Save Definition"))
                {
                    ...
                    var entry = new Core.Mechanics.CustomMechanicEntry
                    {
                        ...
                        Duration = 2.0f // Default fallback
                    };
                ...
                for (int i = drawables.Count - 1; i >= 0; i--)
                {
                    ...
                    if (renamingId == item.UniqueId)
                    {
                        ...
                        if (ImGui.InputText("##Rename", ref renamingBuffer, 64, ImGuiInputTextFlags.EnterReturnsTrue | ImGuiInputTextFlags.AutoSelectAll))
                        {
                            ...
                            renamingId = null;
                            mainWindow.CanvasController.UndoManager.RecordAction(drawables, "Rename Object");
                            mainWindow.CanvasController.InteractionHandler.CommitObjectChanges(new List<BaseDrawable> { item });
                        }
                        ...
                        if (ImGui.BeginDragDropTarget())
                        {
                            ...
                            if (isPayloadValid && payload.Data != null)
                            {
                                ...
                                if (sourceIndex != i)
                                {
                                    ...
                                    mainWindow.CanvasController.UndoManager.RecordAction(drawables, "Reorder Layer");
