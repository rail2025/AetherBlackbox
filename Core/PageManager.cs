...
namespace AetherBlackbox.Core
{
    ...
    public class PageManager
    {
        ...
        public List<BaseDrawable> GetCurrentPageDrawables(float? currentReplayTime = null, float timeWindow = 5.0f)
        {
            var pages = GetAllPages();
            if (pages.Count > 0 && currentPageIndex >= 0 && currentPageIndex < pages.Count)
            {
                var drawables = pages[currentPageIndex].Drawables;
                if (currentReplayTime.HasValue)
                {
                    return drawables.Where(d => Math.Abs(d.ReplayTime - currentReplayTime.Value) <= timeWindow).ToList();
                }
                return drawables;
            }
