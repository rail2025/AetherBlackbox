// AetherBlackbox/Core/UndoManager.cs
using AetherBlackbox.DrawingLogic; // Required for BaseDrawable
using AetherBlackbox.Windows;     // Required for MainWindow.PageData if we store full page states
using System;
...
namespace AetherBlackbox.Core
{
    ...
    public class UndoManager
    {
        ...
        public void InitializeStacks(int pageCount)
        {
            ...
            activeStackIndex = 0;
            Service.PluginLog?.Debug($"[UndoManager] Initialized with {pageCount} undo stacks.");
        }
        ...
        public void SetActivePage(int index)
        {
            ...
            {
                Service.PluginLog?.Error($"[UndoManager] SetActivePage: Invalid index {index}.");
                return;
            }
            activeStackIndex = index;
            Service.PluginLog?.Debug($"[UndoManager] Active page set to {index}.");
        }
        ...
        public void AddStack(int index)
        {
            if (index < 0 || index > undoStacks.Count)
            {
                Service.PluginLog?.Error($"[UndoManager] AddStack: Invalid index {index}.");
                return;
            }
            undoStacks.Insert(index, new Stack<UndoAction>());
            Service.PluginLog?.Debug($"[UndoManager] Added undo stack at index {index}. Total stacks: {undoStacks.Count}");
        }
        ...
        public void RemoveStack(int index)
        {
            if (index < 0 || index >= undoStacks.Count)
            {
                Service.PluginLog?.Error($"[UndoManager] RemoveStack: Invalid index {index}.");
                return;
            }
            undoStacks.RemoveAt(index);
            Service.PluginLog?.Debug($"[UndoManager] Removed undo stack at index {index}. Total stacks: {undoStacks.Count}");
        ...
        public void MoveStack(int fromIndex, int toIndex)
        {
            ...
            {
                Service.PluginLog?.Error($"[UndoManager] MoveStack: Invalid indices. From: {fromIndex}, To: {toIndex}");
                return;
            }
            var stackToMove = undoStacks[fromIndex];
            undoStacks.RemoveAt(fromIndex);
            undoStacks.Insert(toIndex, stackToMove);
            Service.PluginLog?.Debug($"[UndoManager] Moved undo stack from {fromIndex} to {toIndex}.");
        }
        ...
        public void RecordAction(List<BaseDrawable> currentDrawables, string actionDescription)
        {
            ...
            {
                Service.PluginLog?.Error($"[UndoManager] RecordAction: Cannot record, invalid state.");
                return;
            }
            ...
            if (activeStack.Count > 0)
            {
                ...
                if (validCurrentCount == lastAction.PreviousDrawablesState.Count)
                {
                    ...
                    if (currentJson == lastJson)
                    {
                        ...
                        // AetherBlackbox.Plugin.Log?.Debug($"[UndoManager] Action ignored (Duplicate state): {actionDescription}");
                        return;
                    }
                }
            }
            ...
            Service.PluginLog?.Debug($"[UndoManager] Action Recorded on page {activeStackIndex}: {actionDescription}. Stack size: {activeStack.Count}");
        }
        ...
        public List<BaseDrawable>? Undo()
        {
            if (CanUndo())
            {
                ...
                Service.PluginLog?.Debug($"[UndoManager] Undoing Action on page {activeStackIndex}: {lastAction.Description}. Stack size: {activeStack.Count}");
            ...
            Service.PluginLog?.Debug($"[UndoManager] Undo stack empty for page {activeStackIndex}.");
            return null;
        }
        ...
        public void ClearHistory()
        {
            undoStacks.Clear();
            activeStackIndex = 0;
            Service.PluginLog?.Debug("[UndoManager] All undo history cleared.");
        }
        ...
        private void TrimOldestUndo(Stack<UndoAction> activeStack)
        {
            if (activeStack.Count >= MaxUndoLevels)
            {
                ...
                Service.PluginLog?.Debug($"[UndoManager] Trimmed undo stack to {activeStack.Count} items.");
            }
        }
    }
}
